...
6. Review the public proof pack and authored submission materials.
...
Generated account artifacts are intentionally excluded from the public
repository. To validate an authorized local artifact, pass its path explicitly:

```bash
node deliverables/challenge3-target-customer-scouting/prototype-evidence/validate-submission-output.mjs \
  /path/to/private/latest.json
```
